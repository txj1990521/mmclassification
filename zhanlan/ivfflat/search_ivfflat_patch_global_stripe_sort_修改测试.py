...
# ---------- retrieval ----------
TOPG = 800
PATCH_TOPK_PER_QPATCH = 50
TOP_PATCH_IMAGES = 800
RRF_K = 60
...
# ============================================================
def choose_allowedK_from_global(Dg_row: np.ndarray,
                                k_min: int = 300,
                                k_mid: int = 800,
                                k_max: int = 1500,
                                gap_th1: float = 0.02,
                                gap_th2: float = 0.04,
                                spread_th: float = 0.06):
    """
    根据 global 相似度分布决定 allowedK（越不确定 allowedK 越大）
    适用于 IP 相似度（越大越相似）。
    Dg_row: shape (TOPG,) 或 (N,) 的一维数组（g_index.search 的 Dg[0]）
    """
    if Dg_row is None or len(Dg_row) < 2:
        return int(k_max)
    ...
    # spread：top1 - top10（看分布是否平）
    if len(Dg_row) >= 10:
        spread = d0 - float(Dg_row[9])
    else:
        spread = d0 - float(Dg_row[-1])

    # 规则：gap 小 or spread 小 => 不确定 => 放大 allowedK
    if gap < gap_th1 or spread < (0.5 * spread_th):
        return int(k_max)
    if gap < gap_th2 or spread < spread_th:
        return int(k_mid)
    return int(k_min)

def aggregate_patch_hits_stripe_soft(
        ...
        w_cont=0.08,
        only_ptype=1,
        allowed_imgs=None,            # set or None
        outside_penalty=0.20,         # ⭐ 软过滤：allowed 外的惩罚系数
        dedup_bin=True,               # ⭐ bin 内去重
        use_topmean=True              # ⭐ 推荐 True（比 LSE 更抗重复纹理）
):
    meta = patch_meta
    assert isinstance(meta, np.ndarray) and meta.ndim == 2 and meta.shape[1] >= 8

    # img_id -> list[(score, bin)]
    img_hits = {}
    ...
    for pid, s in zip(patch_ids, patch_scores):
        ...
        # soft filter
        w = 1.0
        if allowed_imgs is not None and img_id not in allowed_imgs:
            w = float(outside_penalty)
        ...
        img_hits.setdefault(img_id, []).append((float(s) * w, b))
    ...
    for img_id, sb in img_hits.items():
        ...
        # bin 内去重：同一个 bin 只保留最高分（抗重复纹理刷分）
        if dedup_bin:
            best_per_bin = {}
            for sc, b in sb:
                if (b not in best_per_bin) or (sc > best_per_bin[b]):
                    best_per_bin[b] = sc
            sb2 = [(sc, b) for b, sc in best_per_bin.items()]
        else:
            sb2 = sb
        ...
        sb2.sort(key=lambda x: x[0], reverse=True)
        sb2 = sb2[:topM]
        scores = [x[0] for x in sb2]
        ...
        if use_topmean:
            ...
            m = scores[0]
            v = sum(np.exp((x - m) / max(tau, 1e-6)) for x in scores)
            base = float(m + max(tau, 1e-6) * np.log(v + 1e-9))

        bins = [x[1] for x in sb2]
        ub = sorted(set(bins))

        # coverage / continuity
        cover = min(1.0, len(ub) / 6.0)
...
def aggregate_patch_hits_soft(
        patch_ids, patch_scores, patch_meta,
        top_images=800,
        topM=6,
        tau=0.12,
        allowed_imgs=None,
        outside_penalty=0.20,
        use_topmean=True
):
    ...
    for pid, s in zip(patch_ids, patch_scores):
        ...
        img_scores.setdefault(img_id, []).append(float(s) * w)
    ...
    for img_id, ss in img_scores.items():
        ...
        if use_topmean:
            score = float(np.mean(ss))
        else:
            m = ss[0]
            v = sum(np.exp((x - m) / max(tau, 1e-6)) for x in ss)
            score = float(m + max(tau, 1e-6) * np.log(v + 1e-9))
...
def main():
    ...
    # ---- global search
    Dg, gids = g_index.search(qvec, TOPG)
    global_rank = clean_rank(gids[0].tolist())

    # ---- allowedK 动态
    allowedK = choose_allowedK_from_global(Dg[0],
                                           k_min=300, k_mid=800, k_max=1500,
                                           gap_th1=0.02, gap_th2=0.04,
                                           spread_th=0.06)
    allowed = set(global_rank[:min(len(global_rank), allowedK)])
    print(f"[ALLOW] allowedK={allowedK}  (global_rank size={len(global_rank)})")

    # ---- patch search (按你的 is_stripe2 走也行；更稳是 stripe+grid 都走)
    if is_stripe2:
        D, I = p_index_s.search(q_patch_vecs, PATCH_TOPK_PER_QPATCH)
        S = faiss_scores_from_D(p_index_s, D.astype(np.float32))
        patch_rank = aggregate_patch_hits_stripe_soft(
            I.reshape(-1), S.reshape(-1),
            patch_meta_s,
            top_images=TOP_PATCH_IMAGES,
            allowed_imgs=allowed,
            outside_penalty=0.20,  # 局部图你可以调到 0.35
            dedup_bin=True,
            use_topmean=True
        )
    else:
        ...
        patch_rank = aggregate_patch_hits_soft(
            I.reshape(-1), S.reshape(-1),
            patch_meta_g,
            top_images=TOP_PATCH_IMAGES,
            allowed_imgs=allowed,
    ...
    for ang in angles:
        ...
        print("q_fm.shape:", q_fm.shape)
        q_desc, q_xy = select_query_patches(q_fm)
        print('q_desc.shape:',q_desc.shape)
...
if __name__ == "__main__":
    main()
def run_once(query_img_path: str, out_dir: str = None):
    """
    UI 调用入口：
    - query_img_path: 用户选的图
    - out_dir: 输出目录（可选，默认用 OUT_DIR）
    return: dict with keys: query_path, crop_path, result_path, crop_bgr, result_bgr(optional)
    """
    global QUERY_IMG, OUT_DIR
    ...
    # 复用你 main() 的流程：直接调用 main() 会 print 且保存 result_grid1.png
    main()

    result_path = os.path.join(OUT_DIR, "result_grid1.png")

    # 另外：把 crop 也保存一份，方便 UI 直接读
    qimg0 = imread_unicode(query_img_path)
    qimg, qmask, qimg_raw = crop_by_mmdet_mask_final(
        qimg0,
        score_thr=SEG_SCORE_THR,
        use_classes=SEG_USE_CLASSES,
