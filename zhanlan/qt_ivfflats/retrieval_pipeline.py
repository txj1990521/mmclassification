...
def main():
    ...
    for img_id, fs, gs in top:
        ...
        top_items.append({"path": p, "score": float(fs), "geom": float(gs)})
        imgs.append(imread_unicode(p))
        scores.append(fs)

    out = os.path.join(OUT_DIR, "result_grid.png")
    visualize_grid(qimg, imgs, scores, out, tile=320)

    return out, top_items
...
def run_retrieval(query_img_path: str, out_dir: str, topk: int = 12):
    """
    返回:
      result_grid_path: str
      top_items: list[dict]  (用于 GUI 列表预览)
    """
    global QUERY_IMG, OUT_DIR, TOPK
