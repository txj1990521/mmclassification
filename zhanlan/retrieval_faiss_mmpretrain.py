...
CKPT   = r"D:\zhanlanProject\mmpretrain\work_dirs\simclr_resnet50_8xb32-coslr-200e_in1k_zhanlan\latest.pth"

DATA_ROOT = r"D:\zhanlan\data"      # 图库（递归扫描）
QUERY_IMG = r"D:\zhanlan\query.jpg" # 查询图

OUT_INDEX = r"D:\zhanlan\faiss.index"
OUT_META  = r"D:\zhanlan\faiss_paths.npy"

TOPK = 10
BATCH = 32  # 建库时 batch 大小，4060Ti 一般 16~64 都行
...
def build_model(cfg_path: str, ckpt_path: str, device: str):
    ...
    # 读取 data_preprocessor mean/std/to_rgb
...
def extract_backbone_2048(model, batch_tensor: torch.Tensor):
    """
    batch_tensor: (N,3,224,224)
    return: (N,2048) L2-normalized
    """
    feat_map = model.backbone(batch_tensor)     # (N,C,H,W)
    feat = feat_map.mean(dim=(2,3))             # GAP -> (N,C)
    feat = F.normalize(feat, p=2, dim=1)        # cosine
    return feat
...
def main():
    ...
    qfeat = extract_backbone_2048(model, qx).cpu().numpy().astype("float32")  # (1,D)
    ...
    for r, (s, idx) in enumerate(zip(scores, ids), 1):
        if idx < 0:
            continue
        print(f"{r:02d}  score={float(s):.4f}  {kept_paths[idx]}")
